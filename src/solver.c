...
int solver(const char *output_dir, double *x_vals_mat, double *y_vals_mat, int ni, int nj, int num_points_on_airfoil, const double Mach_inf, const double angle_of_attack_deg, const double density, const double environment_pressure, const double delta_t, const double Gamma, const double epse, const double max_iteration)
{
    ...
    strncpy(temp_word, output_dir, MAXWORD/2);
...
void output_solution_solver(const char *output_dir, double *current_Q, double *U_mat, double *V_mat, double *x_vals_mat, double *y_vals_mat, double *dxi_dx_mat, double *dxi_dy_mat, double *deta_dx_mat, double *deta_dy_mat, int ni, int nj, int i_TEL, int i_LE, int i_TEU)
{
    ...
    /* save data */
    output_mat2D_to_file(x_fp, x_vals_mat, ni, nj);
    output_mat2D_to_file(y_fp, y_vals_mat, ni, nj);
    output_mat2D_to_file(U_fp, U_mat, ni, nj);
    output_mat2D_to_file(V_fp, V_mat, ni, nj);
    output_layer_of_mat3D_to_file(Q0_fp, current_Q, 0, ni, nj);
    output_layer_of_mat3D_to_file(Q1_fp, current_Q, 1, ni, nj);
    output_layer_of_mat3D_to_file(Q2_fp, current_Q, 2, ni, nj);
    output_layer_of_mat3D_to_file(Q3_fp, current_Q, 3, ni, nj);
...
/* wall BC */
    for (i = i_TEL; i <= i_TEU; i++) {
        ...
        calculate_u_and_v(&u_j1, &v_j1, Q, i, 1, ni, nj);
...
/* trailing edge */
    calculate_u_and_v(&u_iTEU_jTEU, &v_iTEU_jTEU, Q, i_TEU, 0, ni, nj);
    calculate_u_and_v(&u_iTEL_jTEL, &v_iTEL_jTEL, Q, i_TEL, 0, ni, nj);
