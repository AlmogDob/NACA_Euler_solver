...
Mach_inf_list = fetch(sqlite(db_file),sprintf('SELECT Mach_inf FROM NACA_data where NACA = %d and delta_t = 1e-5;', NACA));
Mach_inf_list = unique(Mach_inf_list.Mach_inf(:));

figure
hold all
lg = {};
colors = turbo(length(Mach_inf_list))*0.9;
for Mach_index = 1:length(Mach_inf_list)
% for Mach_inf = 0.8
    Mach_inf = Mach_inf_list(Mach_index);
    results = fetch(sqlite(db_file), sprintf('SELECT * FROM NACA_data where NACA = %d and Mach_inf = %f and delta_t = 1e-5;', NACA, Mach_inf));
